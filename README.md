...
## コストコントロール — やらない判断を数値で守る

**🟢 は「触るな」のサインでもある。**

開発コスト（時間・AI API・インフラ）を無駄にする最大の原因は、すでに機能している部分への過剰投賄。KPI スコアが良好な軸は、改善提案が来ても「現状維持」と即答できる。

### KPI 状態 × 投賄判断

| KPI 状態 | 投賄判断 | 理由 |
|----------|---------|------|
| 🔴 | **集中投賄** — 他を止めてでも直す | 放置コストが最大 |
| 🟡 | **計画投賄** — 次サイクルに予算確保 | 悪化する前に手を打つ |
| 🟢 | **投賄凍結** — リファクタ・改善提案を却下 | 壊す方が高くつく |
| ⚪ | **計測コスト払う** — まず数値を取る | 判断できない状態が一番無駄 |

### AI エージェントへの適用

エージェントに「何でも改善して」と指示すると 🟢 の部分にも手を出して費用が膨らむ。KPI スコアを事前に渡すことで作業範囲を限定できる。

```
指示例:
「KPIレポートを確認して、🔴と🟡の軸だけを対象に作業してください。
🟢の軸は触らないでください。」
```

これにより：
- API コール数を削減（無駄な調査・修正を避ける）
- レビューコストを削減（スコープが明確）
- リグレッションリスクを削減（動いている部分に触らない）

### GitHub Actions のコスト最小化

```yaml
# CI 失敗時のみ KPI スキャン (常時スキャンより安い)
on:
  workflow_run:
    workflows: ['ci']
    types: [completed]
...
週次スキャンは `schedule` で十分。毎 push で動かす必要はない。
